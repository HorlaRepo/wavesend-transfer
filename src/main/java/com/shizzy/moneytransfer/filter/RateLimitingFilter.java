...
public class RateLimitingFilter extends OncePerRequestFilter {
    ...
    private Bucket selectBucket(HttpServletRequest request, String identifier) {
        ...
        // Special buckets for financial operations
        if ((path.contains("/transfer") || path.contains("/withdraw") || 
             path.contains("/deposit")) && "POST".equals(method)) {
            return financialTransactionBucket;
        }
        
        // Special bucket for sensitive operations
        if ((path.contains("/password") || path.contains("/kyc") || 
             path.contains("/verification") || path.contains("/security")) && 
            ("POST".equals(method) || "PUT".equals(method))) {
            return sensitiveOperationBucket;
        }
        
        // Use user-specific or IP-specific buckets
        Map<String, Bucket> bucketMap = isAuthenticated ? userBuckets : ipBuckets;
        
        // Create a new bucket for this user/IP if one doesn't exist
        return bucketMap.computeIfAbsent(identifier, k -> 
            isAuthenticated ? authenticatedBucket : anonymousBucket
        );
    }
    ...
    private String getBucketTypeName(HttpServletRequest request, String identifier) {
        ...
        if ((path.contains("/transfer") || path.contains("/withdraw") || 
             path.contains("/deposit")) || path.contains("/initiate") 
             || path.contains("/verify") && "POST".equals(method)) {
